...
This dataset contains `r nrow(ny_noaa)` rows and `r ncol(ny_noaa)` columns. The variables are: `r colnames(ny_noaa)`. There are many missing value `NA` in different variables.
...
For snowfall, 0 is the most common value. This is because it does not snow most of the days in NY. The second most commonly observed value is `NA`, which means there are many missing values.
...
The mean max temperature in January is much lower than the mean temperature in July. All stations tend to follow similar trends of temperature across the year. There are a few outliers. There is one extreme low temperature in July 1988.
...
In the hex plot, most of the data cluster near the center of the distribution. In the ridge plot, the snowfall within a given year has mutiple peaks. Most are between 0 and 30 mm of snow in a year. But there is also other groups arround 50 and nearly 80.
